...
class ConvTemporalGraphical(nn.Module):
    ...
    Args:
        in_channels (int): Number of channels in the input sequence data (x, y)
        out_channels (int): Number of channels produced by the convolution (μx, μy, σx, σy, ρ)
...
class st_gcn(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, use_mdn=False, stride=1, dropout=0, residual=True):
        ...
        assert len(kernel_size) == 2  # 2D kernel
...
class social_stgcnn(nn.Module):
    def __init__(self, n_stgcnn=1, n_txpcnn=1, input_feat=2, output_feat=5, seq_len=6, pred_seq_len=6, kernel_size=3):
        ...
        for j in range(1, self.n_stgcnn):  # n_stgcnn: layers of stgcnn
