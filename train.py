...
dset_train = TrajectoryDataset(data_set + 'train/', obs_len=obs_seq_len, pred_len=pred_seq_len, skip=1, norm_lap_matr=True)

loader_train = DataLoader(dset_train, batch_size=1, shuffle=True, num_workers=0)
# This is irrelative to the args batch size parameter

dset_val = TrajectoryDataset(data_set + 'val/', obs_len=obs_seq_len, pred_len=pred_seq_len, skip=1, norm_lap_matr=True)
...
def train(epoch):
    ...
    for cnt, batch in enumerate(loader_train):
        ...
        if batch_count % args.batch_size != 0 and cnt != turn_point:
            l = graph_loss(V_pred, V_tr) # calculate training loss
            if is_first_loss:
                loss = l
                is_first_loss = False
            else:
            ...
            optimizer.step()
            # Metrics
            loss_batch += loss.item() # accumulate loss
...
def vald(epoch):
    ...
    for cnt, batch in enumerate(loader_val):
        ...
        if batch_count % args.batch_size != 0 and cnt != turn_point:
            l = graph_loss(V_pred, V_tr) # calculate validation loss
            if is_first_loss:
                loss = l
                is_first_loss = False
            else:
